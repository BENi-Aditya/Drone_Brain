...
# Replace with your actual Raspberry Pi IP address below
# Try both Flask (port 5000) and mjpg-streamer (port 8080) URLs
VIDEO_PATHS = [
    "http://192.168.1.103:5000/video_feed",  # Flask default
    "http://10.123.120.207:8080/?action=stream",  # mjpg-streamer default
    "http://192.168.1.103:8080/video_feed"  # If you have a custom path
]
...
# Only keep the latest frame for inference
frame_queue = queue.Queue(maxsize=1)

# --- Inference speedup: skip frames ---
FRAME_SKIP = 2  # Only run inference on every 2nd frame (adjust for speed/accuracy)
...
def inference_worker():
    frame_id = 0
    INFER_EVERY_N = 20  # Increased to skip more frames for speed
    while True:
        try:
            frame = frame_queue.get(timeout=1)
            # --- Increase contrast of frame ---
            contrast_alpha = 1.3  # Contrast control (1.0-3.0)
            contrast_beta = 0     # Brightness control (0-100)
        ...
        if mask_img is None:
            ...
            # Blend overlays on contrast-enhanced frame
...
def main_video_loop():
    ...
    # Try each stream URL with different OpenCV backends
    cap = None
    for url in VIDEO_PATHS:
        print(f"[Main] Trying stream URL: {url} (default backend)")
        cap = cv2.VideoCapture(url)
        if cap.isOpened():
            VIDEO_PATH = url
            break
        print(f"[Main] Trying stream URL: {url} (CAP_FFMPEG backend)")
        cap = cv2.VideoCapture(url, cv2.CAP_FFMPEG)
        if cap.isOpened():
            VIDEO_PATH = url
            break
        print(f"[Main] Trying stream URL: {url} (CAP_ANY backend)")
        cap = cv2.VideoCapture(url, cv2.CAP_ANY)
        if cap.isOpened():
            VIDEO_PATH = url
            break
    if cap is None or not cap.isOpened():
        print("\n[ERROR] Could not open any camera stream.\n" \
              "Please check the following:\n" \
              "- Is your RPi camera server running?\n" \
              "- Can you access the stream in a browser?\n" \
              "- Is the IP and port correct?\n" \
              "- Try both Flask (port 5000) and mjpg-streamer (port 8080) URLs.\n" \
              "- Try restarting the camera server.\n")
        return
    print(f"[Main] Successfully opened stream at {VIDEO_PATH}")
    ...
    while True:
        ...
        if not ret:
            ...
            ret, frame = cap.read()
            if not ret:
                print(f"[Main] Failed to loop video. Exiting.")
                break
        # Feed frame to inference queue only every FRAME_SKIP frames (non-blocking, always keep only latest)
        if frame_id % FRAME_SKIP == 0:
            try:
                # Always keep only the latest frame for inference
        ...
        if key == ord('q'):
            break
        # For fastest inference, minimize wait time
        # Optionally, set wait_time to 1 ms for max speed
        cv2.waitKey(1)
    cap.release()
